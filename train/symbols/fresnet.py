...
def residual_unit_v1(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    """
    use_se = kwargs.get('version_se', 1)  # 获取dict['version_se'],如果关键字不存在设为1,不会更新字典
...
def residual_unit(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs):
    ...
    if uv == 1:
        ...
        if version_input <= 1:
            return residual_unit_v3(data, num_filter, stride, dim_match, name, bottle_neck, **kwargs)  # 用的这个
        else:
...
def resnet(units, num_stages, filter_list, num_classes, bottle_neck, **kwargs):
    ...
    # num_classes: 512, Embedding的个数
    # bottle_neck: False
    bn_mom = kwargs.get('bn_mom', 0.9)  #
    workspace = kwargs.get('workspace', 256)  # 256
    ...
    workspace : int
        Workspace used in convolution operator
    """
    version_se = kwargs.get('version_se', 1)  # 0,是否使用squeeze and excited architecture
    version_input = kwargs.get('version_input', 1)  # 0

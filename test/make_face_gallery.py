...
parser.add_argument('--data_dir', default='../dataset/test/', help='path to dataset.')
parser.add_argument('--ga-model', default='', help='path to load model.')
parser.add_argument('--gallery_dir', default='../gallery/', help='path to face gallery.')
parser.add_argument('--gpu', default=0, type=int, help='gpu id')
parser.add_argument('--det', default=0, type=int, help='mtcnn option, 1 means using R+O, 0 means detect from begining')
parser.add_argument('--threshold', default=0.881, type=float, help='ver dist threshold')
parser.add_argument('--num_per_class', default=8, type=int, help='image number per class')
...
for i in range(0, len(classes)):
    for j in range(args.num_per_class):
        try:
            ...
            print('image:',image_list[i * args.num_per_class + j])
            if pic is None:
                print(image_list[j])
            else:
                ...
                print(i,j,embedding_array.shape)
        except:
